...
public class JDBCExample1 {
	...
	public static void main(String[] args) {
		...
		// java.sql.Statement (전달 & 값 반환 역할)
		// - 1) SQL을 Java -> DB에 전달
		// - 2) DB에서 SQL 수행한 결과를 반환 받아옴 (DB -> Java)
		Statement stmt = null;
		
		// java.sql.ResultSet (stmt가 담아온 결과값을 받아줄 그릇)
		// - SELECT 조회 후 결과(RESULT SET)를 저장하는 객체
		ResultSet rs = null;
		
		try {
			// 2. DriverManager 객체를 이용해서 Connection 객체 생성하지
			...
			String host = "localhost"; // DB 서버 컴퓨터에 IP 또는 도메인주소
										// localhost는 우리가 사용하는 현재 컴퓨터와 같다.
			String port = ":1521"; // DB 서버에 연결하기 위한 PORT 번호
			String dbName = ":XE"; // DBMS 이름 (XE == eXpress Edition)
			
			// jdbc:oracle:thin:@localhost:1521:XE 원한다면 한 객체로 묶어서 만들 수도 있다.
			
			String userName = "kh_hdk"; // 사용자 계정명
			...
			String sql = "SELECT " // 한 줄 넘겨서 쓸 때 SELECT와 같은 특정 절 뒤에는 공백이 있어야 한다. 
	...
	//	☆(중요!!)☆
			...
			while(rs.next()) {
				// 자료형 ResultSet.get자료형(컬럼명 또는 순서);
				...
				System.out.printf("사번 : %s / 이름 : %-5s / 부서코드 : %-5s / 직급코드 : %-5s / "
						+ "급여 : %-5d / 입사일 : %-5s \n",
			...
			try {
				// conn, stmt, rs 세 자원 다 닫아야 한다. (만들어진 역순으로 close 수행하는 것을 권장!)
				if(rs != null) rs.close();
